...
#threads->create("gui_thread",threads->self());
#sub NI_RightClick {
#	my($X, $Y) = Win32::GUI::GetCursorPos();
#	$main->TrackPopupMenu($PopupMenu->{ItemProp},$X, $Y);}
#sub MenConfigure_Click {
#	if ($cfg->{ADMIN_PORT}) {
#		$main->ShellExecute('open',($cfg->{ADMIN_SSL}?"https":"http")."://".
#			($cfg->{ADMIN_AUTH_USER}?"$cfg->{ADMIN_AUTH_USER}:$cfg->{ADMIN_AUTH_PASS}@":"").
#			"localhost:$cfg->{ADMIN_PORT}/admin.tpl",'','',1);
#	} else {
#		$main->ShellExecute('open','notepad.exe',$CONFIG_FILENAME,'',1);
#	}
#}
#sub MenLog_Click {
#	if ($cfg->{ADMIN_PORT}) {
#		$main->ShellExecute('open',($cfg->{ADMIN_SSL}?"https":"http")."://".
#			($cfg->{ADMIN_AUTH_USER}?"$cfg->{ADMIN_AUTH_USER}:$cfg->{ADMIN_AUTH_PASS}@":"").
#			"localhost:$cfg->{ADMIN_PORT}/log.tpl",'','',1);
#	} else {
#		$main->ShellExecute('open',$cfg->{LOGFILE},'','',1);
#	}
#}
#sub MenHelp_Click {
#	$main->ShellExecute('open','..\readme.html','','',1);}
#sub NI_DblClick			{MenConfigure_Click();}
#sub MenExit_Click		{$mthr->kill("INT");}
#sub GuiMain_Terminate 	{$mthr->kill("INT");}
#sub gui_exit			{$extf=1;}
#sub gui_thread {
#	$mthr=shift;
#	$extf=0;
#	$SIG{TERM}=\&gui_exit;
#	$main = Win32::GUI::Window->new( -name   => 'GuiMain');
#	$PopupMenu = new Win32::GUI::Menu(
#		"Item Properties" => "ItemProp",
#		">&Settings" => "MenConfigure",
#		">&Log" => "MenLog",
#		">&Help/About" => "MenHelp",
#		">-" => 0,
#		">&Close HTTPTunnel Client" => "MenExit",
#	);
#	my $icon = {-handle => Win32::GUI::LoadImage('common/client.ico', Win32::GUI::Constants::constant("IMAGE_ICON"),16,16)};
#	bless($icon, "Win32::GUI::Icon");
#	$main->AddNotifyIcon(
#		-name => "NI",
#		-icon => $icon,
#		-tip => "HTTPTunnel Client"
#	);
#	while(!$extf) {Win32::GUI::DoEvents(); usleep(200000);}
#	$main->DESTROY();
#}
#$cfg->{MOD_ZLIB_LOADED}=$cfg->{MOD_ZLIB_AVAILABLE}=1;
#use Compress::Zlib;
##~~endif~~
#~~ifndef~~ WIN32
...
#~~ifdef~~ WIN32
#$cfg->{MOD_LDAP_AVAILABLE}=0;
#$cfg->{MOD_MYSQL_AVAILABLE}=0;
#$cfg->{ENCRYPTION}=$cfg->{MOD_RSA_AVAILABLE}=0;
#~~endif~~
#~~ifndef~~ WIN32
